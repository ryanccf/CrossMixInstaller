...
if ! git remote get-url origin &>/dev/null; then
    ...
    echo "       Run: git remote add origin git@github.com:YOUR_USER/SBCOSInstaller.git"
    exit 1
fi
    ...
    if [ -n "$REPO" ] && command -v gh &>/dev/null; then
        ...
        if [ -n "$RUN_ID" ]; then
            ...
            if $CI_OK; then
                ...
                gh run download "$RUN_ID" --repo "$REPO" --name SBCOSInstaller-Windows --dir /tmp/crossmix-artifacts 2>/dev/null
                if [ $? -eq 0 ] && ls /tmp/crossmix-artifacts/SBCOSInstaller-* &>/dev/null; then
                    cp /tmp/crossmix-artifacts/SBCOSInstaller-* releases/
                    ...
                    sleep 5
                    gh release download "$NEXT_TAG" --repo "$REPO" --pattern "SBCOSInstaller-*.exe" --dir releases/ 2>/dev/null \
                        && echo "  Windows binary downloaded to releases/" \
